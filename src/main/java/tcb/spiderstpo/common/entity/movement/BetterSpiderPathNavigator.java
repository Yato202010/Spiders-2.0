	...
	public void tick() {
		if(!this.isDone()) {
			...
			if(this.targetPosition != null && this.useVanillaBehaviour) {
				// FORGE: Fix MC-94054
				if(!this.targetPosition.closerThan(this.mob.blockPosition(), Math.max(this.mob.getBbWidth(), 1.0D)) && (!(this.mob.getY() > (double) this.targetPosition.getY()) || !(new BlockPos(this.targetPosition.getX(), this.mob.getY(), this.targetPosition.getZ())).closerThan(this.mob.blockPosition(), Math.max(this.mob.getBbWidth(), 1.0D)))) {
